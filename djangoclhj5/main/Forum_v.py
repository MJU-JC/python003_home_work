...
def forum_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'forum' order by addtime desc")
...
def forum_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/djangoclhj5/forum/notify',
            #notify_url=''
        )
...
def forum_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/djangoclhj5/admin/dist/index.html#/forum')
