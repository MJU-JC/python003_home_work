<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="需求时间" prop="xuqiushijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.xuqiushijian" 
                type="datetime"
                :readonly="ro.xuqiushijian"
                placeholder="需求时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xuqiushijian" label="需求时间" prop="xuqiushijian">
              <el-input v-model="ruleForm.xuqiushijian" 
                placeholder="需求时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.xuqiushijian = this.getCurDateTime()
  ...
  methods: {
	...
	this.ruleForm.xiangxishuoming = this.ruleForm.xiangxishuoming.replace(reg,'../../../djangoclhj5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuqiushenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuqiushenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
